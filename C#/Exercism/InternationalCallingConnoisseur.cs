...
public static class DialingCodes
{
    ...
    public static string FindLongestCountryName(Dictionary<int, string> existingDictionary) {
        return existingDictionary.Count > 0 ? existingDictionary.Values.OrderByDescending(countryName => countryName.Length).First() : string.Empty;
        return existingDictionary.Count > 0 ? existingDictionary.Values.OrderByDescending(name => name.Length).First() : string.Empty;
    }
}